...
void countMetaData(const Node* node,
    ...
    // If no subnodes then add up metadata
    if (node->numSubNodes == 0) {
        for (uint32_t metaDataIndex = 0; metaDataIndex < node->numMetaData; ++metaDataIndex) {
            count += node->metaData[metaDataIndex];
        }
    } else { // If subnodes
        // Use metadata as indesx to subnodes to calculate
        for (uint32_t metaDataIndex = 0; metaDataIndex < node->numMetaData; ++metaDataIndex) {
            if ((node->metaData[metaDataIndex] > 0) &&
                (node->metaData[metaDataIndex] <= node->numSubNodes)) {
                countMetaData(node->subNodes[node->metaData[metaDataIndex] - 1], count);
            }
        }
    }
}
